...
contract RewardTracker is BaseToken, ReentrancyGuard {
    using SafeERC20 for IMintableToken;

    uint public immutable PRECISION = 1e30;

    IMintableToken public TOKEN;
    IMintableToken public esTOKEN;
    ...
    function _claim(address user) internal returns (uint) {
        ...
        if (tokenAmount > 0) {
            esTOKEN.safeTransfer(user, tokenAmount);
    ...
    function claim(uint _lockTime) external nonReentrant {
        if (_lockTime != 0) {
